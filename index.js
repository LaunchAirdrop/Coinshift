...
function readProxiesFromFile(filename) {
  try {
    ...
    console.error(chalk.red("failed read file proxy.txt:", err.message));
    return [];
  }
}
  ...
  if (useProxy.toLowerCase() === 'y') {
    ...
    if (proxies.length > 0) {
      ...
      if (proxyUrl.startsWith('http://') || proxyUrl.startsWith('https://')) {
        ...
        console.log(chalk.red("Wrong proxy format. please use http/https atau socks5."));
        return;
      }
      axiosInstance = axios.create({ httpAgent: agent, httpsAgent: agent });
      console.log(chalk.green(`Use proxy: ${proxyUrl}`));
    } else {
      console.log(chalk.red("File proxy.txt empty. run without proxy."));
    }
  } else {
    console.log(chalk.blue("Run without proxy."));
  }
}
...
async function liveCountdown(durationMs) {
  ...
  return new Promise(resolve => {
    const timer = setInterval(() => {
      ...
      process.stdout.write(chalk.yellow(`\rCycle Next ${formatCountdown(remaining)} ...`));
      if (remaining <= 0) {
        clearInterval(timer);
...
async function verifyTask(activityId, headers, privyIdToken) {
  ...
  try {
    ...
    if (!verifyData || !verifyData.record) {
      return { success: false, error: "Empty data record" };
    }
...
try {
    ...
    console.error(chalk.red(`Error check-in for activityId: ${activityId}: ${err.message}`));
    return null;
  }
}
...
async function runCycleOnce(walletKey) {
  ...
  loginSpinner.succeed(chalk.green(" Login success"));
  ...
  try {
    ...
    console.error(chalk.red("Error take XP UserMe:", err.response ? err.response.data : err.message));
  }
  ...
  if (!campaignData) throw new Error("Data of campaign error");
  ...
  let checkinStatus = "Failed Check-in";
  ...
  if (isDailyCheckinClaimed) {
    ...
    const spinnerCheckin = ora(chalk.cyan(`Check-in for Daily Check-in`)).start();
    try {
      const checkInResponse = await performCheckIn(checkinActivityId, campaignHeaders, privyIdToken);
      spinnerCheckin.stop();
      if (!checkInResponse) {
        checkinStatus = "Check-in Failed";
        console.log(chalk.red(`Check-in failed: Not responded from server.`));
      } else if (
        checkInResponse?.data?.verifyActivity?.record?.status?.toUpperCase() === "COMPLETED"
      ) {
        checkinStatus = "Check-in Success";
        console.log(chalk.green("Check-in done."));
      } else if (
        ...
        checkinStatus = "Check-in Failed";
        console.log(chalk.red(`Check-in failed. Respons: ${JSON.stringify(checkInResponse)}`));
      }
    } catch (err) {
      spinnerCheckin.stop();
      checkinStatus = "Check-in Failed";
  ...
  console.log(chalk.cyanBright(`Proxy         : ${proxyUrl || "Nothing"}`));
  console.log(chalk.magenta('============================================================================'));

  console.log(chalk.magenta('\n----------------------------- Claimed Tasks ----------------------------\n'));
  if (claimedTasks.length === 0) {
    console.log(chalk.red('(Nothing task on claimed)\n'));
  } else {
  ...
  if (unclaimedTasks.length === 0) {
    console.log(chalk.red('(Nothing unclaimed task)\n'));
  } else {
...
const accounts = readPrivateKeysFromFile('.env');
  if (!accounts.length) {
    console.error(chalk.red("Nothing privatekey on file .env"));
    process.exit(1);
  }
  ...
  while (true) {
    ...
    for (const key of accounts) {
      console.log(chalk.cyan(`Proccessing account: ${shortAddress((new Wallet(key)).address)}\n`));
      try {
        await runCycleOnce(key);
      } catch (err) {
        console.error(chalk.red(`Error account ${shortAddress((new Wallet(key)).address)}: ${err.message}`));
      }
...
function readPrivateKeysFromFile(filename) {
  try {
    ...
    console.error(chalk.red("Failed to read file .env:", err.message));
    process.exit(1);
  }
}

mainLoopRoundRobin().catch(err => console.error(chalk.red(" error fatal:", err.message)));
